...
<small><small>
**Figuur 1.** *[Workflow](https://github.com/J4nBoersma/Jan-Boersma/blob/main/Assets/Workflow.png) voor de Transcriptomics analyse van de genen en pathways bij RA patiënten en controle groep.*</small></small>

In dit onderzoek is gebruikgemaakt van RNA-sequencingdata afkomstig uit synoviumbiopten. De dataset bestaat uit acht samples: vier controles zonder reumatoïde artritis (RA) en vier RA-patiënten met een ziekteduur van meer dan 12 maanden. Alle patiënten waren ACPA-positief, terwijl de controles ACPA-negatief waren. De data zijn afkomstig uit [Platzer et al. (2019)](https://github.com/J4nBoersma/Jan-Boersma/blob/main/Bronnen/Platzer%20et%20al.%20(2019).pdf)
De analyse is uitgevoerd in R. Eerst is het humane referentiegenoom GRCh38.p14 (GCF_000001405.40) geïndexeerd met het R-package Rsubread v2.24.0 [Liao et al., 2019](Packages/Rsubread.pdf). Vervolgens zijn paired-end reads uitgelijnd tegen dit referentiegenoom, waarna voor alle samples BAM-bestanden zijn gegenereerd.
Op basis van deze alignments is met featureCounts een gen-level countmatrix opgesteld met behulp van een GTF-annotatiebestand. Deze matrix is vervolgens gebruikt als input voor downstream analyse in DESeq2 v1.50.2 [Love et al., 2014](Packages/DESeq2.pdf). Na normalisatie is een differentiële expressieanalyse uitgevoerd om genen te identificeren met significante expressieveranderingen tussen de RA- en controlegroep (padj < 0.05, |log2FC| > 1).
Voor visualisatie is een volcano plot gemaakt met EnhancedVolcano v1.28.2 [Blighe et al., 2022](Packages/EnhancedVolcano.pdf). Daarnaast zijn significant differentieel tot expressie komende genen geselecteerd voor functionele verrijkingsanalyses. Gene Ontology (GO)-analyse is uitgevoerd met clusterProfiler v4.18.4 [Wu et al., 2021](Packages/clusterProfiler.pdf), waarbij biologische processen, cellulaire componenten en moleculaire functies zijn onderzocht. KEGG pathway-analyse is toegepast om verrijkte signaalroutes te identificeren. Ten slotte zijn de differentieel geëxprimeerde genen geprojecteerd op de KEGG Rheumatoid arthritis-pathway (hsa05323) met behulp van pathview versie 1.50.0 [Luo & Brouwer, 2013](Packages/Pathview.pdf). Hiermee zijn de veranderingen in genexpressie binnen de context van de RA-gerelateerde pathway gevisualiseerd.
...
- Firestein, G. S., McInnes, I. B., University of California San Diego, & Institute of Infection Immunity and Inflammation, University of Glasgow. (2017). Immunopathogenesis of Rheumatoid Arthritis. In Immunity (Vol. 46, p. 183) [Journal-article]. https://doi.org/10.1016/j.immuni.2017.02.006
