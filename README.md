...
Op basis van deze alignments is met featureCounts een gen-level countmatrix opgesteld met behulp van een GTF-annotatiebestand. Deze matrix is vervolgens gebruikt als input voor downstream analyse in DESeq2 v1.50.2 (Love et al., 2014). Na normalisatie is een differentiële expressieanalyse uitgevoerd om genen te identificeren met significante expressieveranderingen tussen de RA- en controlegroep (padj < 0.05, |log2FC| > 1).
